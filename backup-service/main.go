...
func main() {
	...
	// Initialize log file
	logFilePath := filepath.Join(config.BackupDirectory, "docker-h5p-backup.log")
...
func handleBackup(config *Config, backupType string, now time.Time) {
	...
	backupFile := filepath.Join(backupPath, fmt.Sprintf("h5p-backup-%s-%s.tar", backupType, now.Format("02.01.2006")))
